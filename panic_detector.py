...
def get_market_data():
    ...
    sp_current = float(hist["Close"].iloc[-1])
    sp_peak = float(hist["Close"].max())
...
def check_buy_zone(data):
    ...
    if data["drawdown"] < -6 and data["vix_price"] > 25:
...
def check_oil_spike():
    ...
    try:
        ...
        if pct > 5 and last_oil_alert_day != today:
...
def maybe_send_crypto_update(now):

    global last_crypto_update

    if last_crypto_update is None or (now - last_crypto_update) > timedelta(hours=2):
